...
for resized in pyramid_gaussian(img, downscale=6): # loop over each layer of the image that you take!
    ...
    for (x, y, window) in sliding_window(resized, stepSize=10, windowSize=(winW, winH)):
        ...
        # Check if the window has the correct number of channels
        if len(window.shape) != 3 or window.shape[2] != 3:
            # Skip this window if it doesn't have 3 channels (assuming RGB)
            continue
        
        window = color.rgb2gray(window) # convert sliding window to gray
        ...
        if pred == 1:
            if model.decision_function(fds) > 3:  # set a threshold value for the SVM prediction i.e. only firm the predictions above probability of 0.6
...
if k == 27:             #wait for ESC key to exit
    ...
    cv2.imwrite('image/saveImg.jpg',img)
    cv2.destroyAllWindows()
